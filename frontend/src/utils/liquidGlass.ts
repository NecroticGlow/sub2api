/**
 * Liquid glass helpers shared by the LiquidGlass component.
 *
 * Ported from https://github.com/rdev/liquid-glass-react (MIT).
 * The displacement map encodes per-pixel offsets sampled by an SVG
 * feDisplacementMap filter to bend the backdrop near the glass edges.
 */

// Standard displacement map (radial edge falloff), inlined as a data URI so
// the effect works offline and without extra asset requests.
export const liquidGlassDisplacementMap =
  'data:image/jpeg;base64,'

let cachedSupport: boolean | null = null

/**
 * SVG displacement filters applied on top of backdrop-filter only render on
 * Chromium engines. Firefox ignores the url() filter and Safari produces
 * artifacts, so both fall back to plain frosted glass.
 */
export function supportsLiquidGlassDisplacement(): boolean {
  if (cachedSupport !== null) return cachedSupport
  if (typeof navigator === 'undefined') return false
  const ua = navigator.userAgent.toLowerCase()
  const isFirefox = ua.includes('firefox')
  const isSafari =
    ua.includes('safari') &&
    !ua.includes('chrome') &&
    !ua.includes('chromium') &&
    !ua.includes('android')
  cachedSupport = !isFirefox && !isSafari
  return cachedSupport
}

let uidCounter = 0

/** Unique id generator for per-instance SVG filter elements. */
export function nextLiquidGlassId(): string {
  uidCounter += 1
  return `liquid-glass-filter-${uidCounter}`
}
