...
func TestGetFallbackPricing_FamilyMatching(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "openai unknown no fallback", model: "gpt-unknown-model", expectNilPricing: true},
		{
			...
			expectedInput:     4.5 / 7e6,
			expectedOutput:    floatPtr(13.5 / 7e6),
			expectedCacheRead: floatPtr(0.15 / 7e6),
		},
		{
			name:              "deepseek v4 flash",
			model:             "deepseek-v4-flash",
			expectedInput:     1.5 / 7e6,
			expectedOutput:    floatPtr(4.5 / 7e6),
			expectedCacheRead: floatPtr(0.05 / 7e6),
		},
		{
